...
title: "Send message - Server API - Documentation - Feishu Open Platform"
---
...
[Prerequisites](https://open.feishu.cn/document/server-docs/im-v1/message/create#347504e0 "Prerequisites")

[Limitation of use](https://open.feishu.cn/document/server-docs/im-v1/message/create#527e6af5 "Limitation of use")

[Request](https://open.feishu.cn/document/server-docs/im-v1/message/create#request "Request")

[Request header](https://open.feishu.cn/document/server-docs/im-v1/message/create#requestHeader "Request header")

[Query parameters](https://open.feishu.cn/document/server-docs/im-v1/message/create#queryParams "Query parameters")

[Request body](https://open.feishu.cn/document/server-docs/im-v1/message/create#requestBody "Request body")

[Request example](https://open.feishu.cn/document/server-docs/im-v1/message/create#requestExample "Request example")

[Response](https://open.feishu.cn/document/server-docs/im-v1/message/create#response "Response")

[Response body](https://open.feishu.cn/document/server-docs/im-v1/message/create#responseBody "Response body")

[Response body example](https://open.feishu.cn/document/server-docs/im-v1/message/create#responseBodyExample "Response body example")

[Error code](https://open.feishu.cn/document/server-docs/im-v1/message/create#errorCode "Error code")

[230099 Sub-error code](https://open.feishu.cn/document/server-docs/im-v1/message/create#e437da66 "230099 Sub-error code")

# Send a message

Call this interface to send a message to a specified user or group chat. Supported message types include text, rich text, cards, group business cards, personal business cards, pictures, videos, audio, files, and emoticons.

Try It

## Prerequisites

- The application needs to enable [bot capability](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-enable-bot-ability). The capability will take effect only after the release of a new version. Refer to [release an app](https://open.feishu.cn/document/home/introduction-to-custom-app-development/self-built-application-development-process#baf09c7d).
- When sending messages to users, the user needs to be within the [availability range](https://open.feishu.cn/document/home/introduction-to-scope-and-authorization/availability) of the bot.
- When sending a message to a group, the robot needs to be in the group and have speaking rights in the group.

## Limitation of use

- To avoid frequent message sending causing disturbance to users, the frequency limit for sending messages to the same user is 5 QPS, and the frequency limit for sending messages to the same group is 5 QPS shared by bots in the group.

- This interface can only be called by application robots created in the developer backend. Group custom robots cannot call this interface. For more information about how to use group custom robots, see [Custom Robot Usage Guide](https://open.feishu.cn/document/ukTMukTMukTM/ucTM5YjL3ETO24yNxkjN).
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| receive\_id\_type | string | Yes | User ID categories<br>**Example value**: "open\_id"<br>**Optional values are**:<br>- `open_id`：Identifies a user to an app. The same user has different Open IDs in different apps. [How to get Open ID](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)<br>- `union_id`：Identifies a user to a tenant that acts as a developer. A user has the same Union ID in apps developed by the same developer, and has different Union IDs in apps developed by different developers. A developer can use Union ID to link the same user's identities in multiple apps. [How to get Union ID](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-union-id)<br>- `user_id`：Identifies a user to a tenant. The same user has different User IDs in different tenants. In one single tenant, a user has the same User ID in all apps （including store apps）. User ID is usually used to communicate user data between different apps. [How to get User ID](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id)<br>- `email`：Identifies users by "email", which is the user's actual email address<br>- `chat_id`：Identifies group chats by chat\_id. You can call the API [Search for groups visible to a user or bot](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat/search) to obtain the chat\_id value. For group ID description, refer to [Group ID description](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-id-description).<br>**When the value is `user_id`, the following field scopes are required**:<br>Obtain user ID<br>Only custom apps |

### Request body

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| receive\_id | string | Yes | The ID of the message recipient. The ID type is consistent with the value of the query parameter `receive_id_type`.<br>**Notice**:<br>- When sending a message to a user, the user needs to be in the bot's [availability scope](https://open.feishu.cn/document/home/introduction-to-scope-and-authorization/availability). For example, if you need to send a message to all employees of the company, you need to set the application's availability scope to all employees.<br>- When sending a message to a group, the bot needs to be in the group and have the right to speak in the group.<br>- If the message recipient is a user, it is recommended to use the user's `open_id`.<br>**Example value**: "ou\_7d8a6e6df7621556ce0d21922b676706ccs" |
| msg\_type | string | Yes | Message type.<br>**Optional values**:<br>- text<br>  <br>- post<br>  <br>- image<br>  <br>- file<br>  <br>- audio<br>  <br>- media<br>  <br>- sticker<br>  <br>- interactive<br>  <br>- share\_chat (The shared group card is valid for 7 days)<br>  <br>- share\_user<br>  <br>- system: This type only supports pushing system messages in bot single chats and not in group chats. For example, as shown in the image below to highlight a new conversation.<br>  <br>  ![image.png](https://sf3-cn.feishucdn.com/obj/open-platform-opendoc/5993139c10a955d8dd8479faa0ec335b_tOskM2Gh1F.png)<br>  <br>For detailed description of different message types, see [Send message content](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/im-v1/message/create_json).<br>**Example value**: "text" |
| content | string | Yes | Message content, a string serialized from a JSON structure. The value of this parameter corresponds to `msg_type`. For example, if the value of `msg_type` is `text`, this parameter needs to pass in text type content.<br>**Note:**<br>- JSON strings need to be escaped. For example, the line break `\n` is escaped as `\\n`.<br>- The maximum size of a text message request body cannot exceed 150 KB.<br>- The maximum size of a card message or rich text message request body cannot exceed 30 KB.<br>  - If a card template (template\_id) is used to send a message, the actual size also includes the card data size corresponding to the template.<br>  - If the message contains style tags, the actual message body length will be greater than the request body length you entered.<br>- For images, you need to [upload the image](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/image/create) first, and then use the image's Key to send a message.<br>- Audio, video, and files need to be [uploaded](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/file/create) first, and then the file's key is used to send a message. Note that you cannot use the file\_token returned by the cloud document [upload material](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/upload_all) interface.<br>For different types of message content formats and usage restrictions, see [Sending Message Content](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/im-v1/message/create_json).<br>**Example value**: "`{\"text\":\"test content\"}`" |
| uuid | string | No | A custom unique string sequence used to request deduplication when sending messages. For requests with the same uuid, at most one message can be successfully sent within 1 hour.<br>**Note**: You can customize the parameter value by referring to the example value. When sending different message contents, if this parameter is passed in, you need to change the value of the parameter in each request.<br>**Example value**: "a0d69e20-1dd1-458b-k525-dfeca4015204"<br>**Data validation rules**:<br>- Maximum length: `50` characters |
...
curl-i-X POST 'https://open.feishu.cn/open-apis/im/v1/messages?receive\_id\_type=open\_id' \

**Send card request example**

Below is an example of sending a card constructed by JSON. For other methods of sending cards, refer to [Message Content Structure - Cards](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/im-v1/message/create_json#3ea4c2d5).
...

{
	...
	"content": "{\"elements\":[{\"tag\":\"markdown\",\"content\":\"normal text\\n\\n // at specified user\\n <at id=\\\"ou_cf986xxxx\\\"></at> // use open_id @ specify person\\n <at id=\\\"ou_cf986xxxx\\\"></at> // use user_id @ specify person\\n <at email=\\\"xxxx@example.com\\\"></at> // use email address @ specify person\\n // at everyone\\n <at id=\\\"all\\\"></at>\"}]}"
}
```
...
Golang demo refer to [Robot automatic pull group alarm tutorial (including Go sample code)](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message-development-tutorial/determine-the-api-and-event-to-call)
...
| --- | --- | --- | --- |
| 400 | 230001 | Your request contains an invalid request parameter. | Parameter error: Check whether the input parameters are filled in correctly based on the error information actually returned by the interface and the interface documentation. Check the [How to solve error 230001](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-resolve-error-230001) document based on the `ext` information for troubleshooting. |
...
| 400 | 230015 | P2P chat can NOT be shared. | You cannot share business cards for individual chats. You can only share business cards for group chats or personal business cards of users. |
| 400 | 230017 | Bot is NOT the owner of the resource. | The robot is not the owner of the resource. The robot can only send files uploaded by itself. For file upload methods, see [Upload File](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/file/create). |
...
| 400 | 230019 | The topic does NOT exist. | The current topic does not exist and the operation cannot be performed. |
...
| 400 | 230029 | User has resigned. | The specified user has resigned and cannot send messages. |
| 400 | 230034 | The receive\_id is invalid | The message receiver ID (receive\_id) is invalid. Please pass in an ID that matches the receive\_id\_type type and ensure that the ID is correct. |
...
| 400 | 230036 | Tenant crypt key has been deleted. | The tenant encryption key has been deleted, making the data encrypted by the key unavailable. Please contact the enterprise administrator for resolution. |
...
| 400 | 232009 | Your request specifies a chat which has already been dissolved. | The specified group has been disbanded and messages cannot be sent. Please refer to [Group ID Description](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-id-description#2c14aa7f) to obtain an available group ID and try again. |
...
| 11310 | Failed to create card content, ext=ErrCode: 11310; ErrMsg: %v; code:230099 | - If the ErrMsg indicates "template is not visible to app, please confirm whether the app is allowed to use this template," ensure that the owner of the card is within the [availability scope](https://open.feishu.cn/document/home/introduction-to-scope-and-authorization/availability) of the calling app;<br>- If the ErrMsg indicates "element exceeds the limit," it means the card in JSON 2.0 structure exceeds the limit of 200 components or elements. Please reduce the number of components or elements and try again.<br>- If the ErrMsg indicates "card action is lock," it means the card with a JSON 2.0 structure is currently in interaction and cannot be updated. |
...
| 200732 | Failed to create card content, ext=ErrCode: 200732; ErrMsg: the actual type of the variable is inconsistent with the specified type in the template. Please check the type of the variable; ErrorValue: %v, specifiedType: %v, actualType: %v | Card variable type error. Please ensure that the data type of the variables used when sending the template card matches the data type specified when constructing the template. For the new version of cards (JSON 2.0 structure), it is recommended to send the errMsg error information to the [AI Assistant](https://open.feishu.cn/search?from=header&page=1&pageSize=10&q=Failed+to+create+card+content,+ext=ErrCode:+200732;+ErrMsg:+the+actual+type+of+the+variable+is+inconsistent+with+the+expected+type+in+the+template.+please+check+the+type+of+the+variable;+ErrorValue:+varName:+text_var_1,+expectType:+string,+actualType:+bool;&topicFilter=) for targeted advice on specific error variables. |
| 200737 | Failed to create card content, ext=ErrCode: 200737; ErrMsg: the format of the variable is incorrect. Please check the format of the variable; ErrorValue: %v, specifiedFormat: %v, actualFormat: %v | The format of the specific value passed for the variable does not match the required format for that variable. It is recommended to send the errMsg error information to the [AI Assistant](https://open.feishu.cn/search?from=header&page=1&pageSize=10&q=Failed+to+create+card+content,+ext=ErrCode:+200737;+ErrMsg:+the+format++of+the+variable+is+incorrect.+please+check+the+format+of+the+variable;+ErrorValue:+varName:+text_var_1,+specifiedFormat:+yyyy-MM-dd,+actualFormat:+yyyy&topicFilter=) for targeted advice on specific error variables. |
...
| 300240 | Failed to create card content, ext=ErrCode: 300240; ErrMsg: This application cannot retrieve audio information. Please check if the application used for uploading audio is the same as the one used for sending messages; | The `file_key` of the audio in the audio component is invalid. Please ensure:<br>- You have called the [Upload File](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/file/create) API to obtain the `file_key` of the audio.<br>- The application used to call the [Upload File](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/file/create) API is the same as the one used to send the card. |
| 300230 | Failed to create card content, ext=ErrCode: 300230; ErrMsg: duplicate audio id; | Within the same card, the value of the `audio_id` field in the audio component must be globally unique. Please check if there are duplicate values of `audio_id` in the card JSON. |
| 300220 | Failed to create card content, ext=ErrCode: 300220; ErrMsg: audio elem don't support forward; | Cards containing audio components do not support forwarding. Please specify `enable_forward` as `false` in the `config` configuration of the card JSON. |
...
[Explain](https://open.feishu.cn/app/ai/playground?query=Based%20on%20the%20doc%20at%20https%3A%2F%2Fopen.feishu.cn%2Fdocument%2Fserver-docs%2Fim-v1%2Fmessage%2Fcreate%2C%20explain%20what%20%22%22%20means&from=doc_text_select) Document Error Correction
...
[Previous:Receive message content structure](https://open.feishu.cn/document/server-docs/im-v1/message-content-description/message_content) [Next:Reply message](https://open.feishu.cn/document/server-docs/im-v1/message/reply)
...
RUN [Go to API Explorer](https://open.feishu.cn/api-explorer?from=op_doc&apiName=create&project=im&resource=message&version=v1)
