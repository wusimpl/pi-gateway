...
title: "Get custom app tenant_access_token - Server API - Documentation - Feishu Open Platform"
---
...
Get custom app tenant\_access\_token
...
[Precautions](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#d258cc64 "Precautions")

[Request](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#7dbabeae "Request")

[Request header](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#20964697 "Request header")

[Request body](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#422ed976 "Request body")

[Request body example](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#82f133c0 "Request body example")

[Response](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#aa567d02 "Response")

[Response body](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#0797b7de "Response body")

[Response body example](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#30f4b514 "Response body example")

[Error code](https://open.feishu.cn/document/server-docs/authentication-management/access-token/tenant_access_token_internal#4b37c186 "Error code")

# Custom apps get tenant\_access\_token

Custom apps can obtain `tenant_access_token` through this interface.

## Precautions

The maximum validity period of `tenant_access_token` is 2 hours.

- If called when the validity is less than 30 minutes, this interface will return a new `tenant_access_token`, resulting in two valid tokens existing simultaneously.
- If the validity period is not less than 30 minutes, calling this interface will return the original `tenant_access_token`.
...
| app\_secret | string | Yes | Application key, obtained after creating the application. For a detailed introduction to `app_secret`, please refer to the introduction of [General parameters](https://open.feishu.cn/document/ukTMukTMukTM/uYTM5UjL2ETO14iNxkTN/terminology).<br>**Example value:** "dskLLdkasdjlasdKK" |
...
| tenant\_access\_token | string | The access token. |
| expire | int | The expiration time of tenant\_access\_token, in seconds. |
...
For a detailed description of error codes, see Introduction to [Common error codes](https://open.feishu.cn/document/ukTMukTMukTM/ugjM14COyUjL4ITN).

[Explain](https://open.feishu.cn/app/ai/playground?query=Based%20on%20the%20doc%20at%20https%3A%2F%2Fopen.feishu.cn%2Fdocument%2Fserver-docs%2Fauthentication-management%2Faccess-token%2Ftenant_access_token_internal%2C%20explain%20what%20%22%22%20means&from=doc_text_select) Document Error Correction
...
[Previous:Log out](https://open.feishu.cn/document/authentication-management/login-state-management/logout) [Next:Get custom app app\_access\_token](https://open.feishu.cn/document/server-docs/authentication-management/access-token/app_access_token_internal)
