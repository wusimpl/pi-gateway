...
const DOCX_IMAGE_GUIDELINES = [
  "如果要在飞书文档里直接嵌图，正文里必须使用真正的图片写法：Markdown `![alt](url)`，或 HTML `<img src=\"...\">`。",
  "支持 `http(s)` 图片地址和 `data:image/...`；裸图片 URL 或普通 Markdown 链接 `[文字](url)` 不算图片，不要把它们当成可嵌图输入。",
  "如果正文里包含可直接访问的真实图片，默认应让文档直接嵌图，不要主动改成“保留原图链接”。",
  "嵌图依赖飞书应用已经开通图片上传权限；只有在权限没开、图片不是直链、下载失败、返回的不是图片、需要登录，或写入失败时，才退回成图片链接，并在最终回复里明确说清原因。",
];
