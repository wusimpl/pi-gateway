...
async function handleApiRequest(
  ...
  if (url.pathname === "/admin/api/pages/tools" && req.method === "GET") {
    const targetKey = url.searchParams.get("targetKey") ?? "";
    if (!targetKey) {
      sendJson(res, 400, { error: "TARGET_REQUIRED", message: "请先选择私聊或群聊。" });
      return;
    ...
    try {
      sendJson(res, 200, await deps.pages?.getToolsPage(targetKey) ?? {});
    } catch (error) {
      if ((error as Error).message === "ADMIN_TARGET_NOT_FOUND") {
        sendJson(res, 404, { error: "TARGET_NOT_FOUND", message: "目标不存在。" });
        return;
  ...
  if (url.pathname === "/admin/api/pages/control" && req.method === "GET") {
    const targetKey = url.searchParams.get("targetKey") ?? "";
    if (!targetKey) {
      sendJson(res, 400, { error: "TARGET_REQUIRED", message: "请先选择私聊或群聊。" });
      return;
    ...
    try {
      sendJson(res, 200, await deps.pages?.getControlPage(targetKey) ?? {});
    } catch (error) {
      if ((error as Error).message === "ADMIN_TARGET_NOT_FOUND") {
        sendJson(res, 404, { error: "TARGET_NOT_FOUND", message: "目标不存在。" });
        return;
  ...
  if (url.pathname === "/admin/api/pages/skills" && req.method === "GET") {
    const targetKey = url.searchParams.get("targetKey") ?? "";
    if (!targetKey) {
      sendJson(res, 400, { error: "TARGET_REQUIRED", message: "请先选择私聊或群聊。" });
      return;
    ...
    try {
      sendJson(res, 200, await deps.pages?.getSkillsPage(targetKey) ?? {});
    } catch (error) {
      if ((error as Error).message === "ADMIN_TARGET_NOT_FOUND") {
        sendJson(res, 404, { error: "TARGET_NOT_FOUND", message: "目标不存在。" });
        return;
