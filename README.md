...
- 命令支持：`/new`、`/reset`、`/status`、`/context`、`/skills`、`/model`、`/models`、`/stop`、`/restart`
- 优雅关停
...
`./start.sh` 会优先尝试启动已存在的 `pi-gateway` 进程；如果 PM2 里还没有这个进程，则自动执行 `npm run dev` 并注册为 `pi-gateway`。

## 限制

- 仅支持私聊，不支持群聊
- 入站消息目前仅支持 `text`、`post`、`image`、`audio` 四类私聊消息；文件、视频、名片、表情等其他类型会被忽略
- 单实例部署，消息去重、运行锁和重启排空状态仅覆盖当前进程
- 同一用户的消息会按队列串行处理，不会并发执行
